#!/bin/sh
# This is a comment!
echo 1. curl -s -k --data 'username=user&password=test&grant_type=password&client_id=demo-client&client_secret=8a899f1a-e391-4bbf-b29f-35f103fda84d' http://localhost:8080/auth/realms/demo/protocol/openid-connect/token
OTP_SEND_TOKEN="$(curl -s -k --data 'username=user&password=test&grant_type=password&client_id=demo-client&client_secret=8a899f1a-e391-4bbf-b29f-35f103fda84d' http://localhost:8080/auth/realms/demo/protocol/openid-connect/token |  jq -r  '.mfa_token')"; \
echo Reveived token
